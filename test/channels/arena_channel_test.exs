defmodule Countdown.ArenaChannelTest do
  use Countdown.ChannelCase, async: true
   ...
   test "should win the game and reset the counter when reaches the counter limit", %{socket: socket} do
     # At this moment counter value should be counter limit - 1
     Counter.set(Counter.limit - 1)
     # Then, when sending "count" event to channel it should update counter (to 1)
     ref = push socket, "count", %{}
     # Replying the counter
     assert_reply ref, :ok, %{won: true, counter: 0}
     # Let's ensure counter has been reset
     assert 0 = Counter.value
   end
end
